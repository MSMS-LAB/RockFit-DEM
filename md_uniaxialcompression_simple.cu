...
__device__ void dd_FrictionForce(const float* __restrict__ V, float* __restrict__ F, const uint_fast32_t &N, const uint_fast32_t& idx, const float &fz, const float& mu)
{
...
__global__ void d_UniaxialCompression_simple(const float* __restrict__ R, const float* __restrict__ V, float* __restrict__ F, float* __restrict__ FL, const uint_fast32_t N, const float3 c, const float RR, const float Hd2, const float C, const float mu, const float Zcut)
{
	...
	while (idx < N)
	{
		...
		if (r.x * r.x + r.y * r.y < RR && R[idx + 2 * N] < Zcut)
		{
			...
			{
				float fz = (Hd2 - r.z) * C;
				dd_FrictionForce(V, F, N, idx, fz, mu);
				F[idx + 2 * N] += fz;
				sft -= fz;
