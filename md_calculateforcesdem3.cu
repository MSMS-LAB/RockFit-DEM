...
__device__ void dd_CheckBreakConditionDEM_2(float& Sc, float& Mc, float3* __restrict__ Oijt,	float3* __restrict__ Mijn, float3* __restrict__ Mijt,
...
__device__ void dd_Calculate_rijm_nij_2(const float* __restrict__ R, const uint_fast32_t& idx, const uint_fast32_t& jdx, const uint_fast32_t& N, float3& nij, float& rijm)
{
	float3 rij;
	float _1d_rm;
	rij.x = R[jdx] - R[idx];
...
__device__ void dd_Calculate_M123_bpm_2(const float* __restrict__ R, const float* __restrict__ W, float3* __restrict__ Rij, const uint_fast32_t& kdx, const uint_fast32_t& idx, const uint_fast32_t& jdx, const uint_fast32_t& N, const float dt, const float3& nij, float3& M1, float3& M2, float3& M3)
{
...
__device__ void dd_Calculate_fijn_bpm_2(const float* __restrict__ _1d_iL, float& Sc, float3& Fijn, const uint_fast32_t& kdx, const float3& nij, const float& rijm, const float& m_E, const float& b_r)
{
	// calculate the force
...
__device__ void dd_Calculate_mijn_bpm_2(const float* __restrict__ W, const float* __restrict__ _1d_iL, float3* __restrict__ Mijn, float3& Mijnn, const uint_fast32_t& kdx, const uint_fast32_t& idx, const uint_fast32_t& jdx, const uint_fast32_t& N,
...
__device__ void dd_Calculate_mijt_bpm_2(const float* __restrict__ W, const float* __restrict__ _1d_iL, float3* __restrict__ Mijt, float3& Mijtt, const uint_fast32_t& kdx, const uint_fast32_t& idx, const uint_fast32_t& jdx, const uint_fast32_t& N,
...
__device__ void dd_Calculate_fijt_mijunsym_bpm_2(const float* __restrict__ R, const float* __restrict__ V, const float* __restrict__ W, 
...
__device__ void dd_Calculate_fijn_ppHM_2(const float* __restrict__ V, const float* __restrict__ W, float3& Fijn,
...
__device__ void dd_Calculate_fijtmijt_ppHM_2(float3* __restrict__ Oijt, float3& Fijt, float3& Mijt,
...
__device__ void dd_Calculate_mijroll_ppHM_2(const float* __restrict__ W, float3& Mijadd, const uint_fast32_t& kdx, const uint_fast32_t& idx, const uint_fast32_t& N, const float& m_muroll, const float& fijnm)
{
	float3 wi;
	wi.x = W[idx];
	wi.y = W[idx + N];
	...
	while (idx < N)
	{
		...
		for (kdx = idx * IonP; kdx < (idx + 1) * IonP; ++kdx)
		{
			...
			if (jdx != UINT_FAST32_MAX && ILtype[kdx] == 0)
			{
				float3 nij;
				float rijm;
				dd_Calculate_rijm_nij_2(R, idx, jdx, N, nij, rijm);
				if (rijm < 2.0f * rP)
				{
					...
					float Kt, fijnm;
					//CHECK Overlap!
					dd_Calculate_fijn_ppHM_2(V, W, fs, kdx, idx, jdx, N, m_E, m_G, p_A, mP, rP, nij, rijm, vijt, Kt, fijnm);
					//printf("pp %e %e %e %e %e\n", 1.0f*mP, 1.0f*rP);
					...
					//Fijn[kdx].y = 0;
					//Fijn[kdx].z = 0;
					dd_Calculate_fijtmijt_ppHM_2(Oijt, fs, ms, kdx, nij, m_mu, p_A, mP, rP, dt, vijt, Kt, fijnm);
					//printf("pp %e %e %e |\n", Kt);
					//Fijt[kdx].x = 0;
					//Fijt[kdx].y = 0;
					//Fijt[kdx].z = 0;
					...
					//Mijt[kdx].y = 0;
					//Mijt[kdx].z = 0;
					dd_Calculate_mijroll_ppHM_2(W, ms, kdx, idx, N, m_muroll, fijnm);
					//Mijadd[kdx].x = 0;
					//Mijadd[kdx].y = 0;
					//Mijadd[kdx].z = 0;	
				}
	...
	{		
		...
		for (kdx = idx * IonP; kdx < (idx + 1) * IonP; ++kdx)
		{
			...
			if (jdx != UINT_FAST32_MAX && ILtype[kdx] == 1)
			{
				...
				//printf("In %u %u %u | %e %e %e | %e %e %e | %e %e %e\n", kdx, idx, jdx, rij.x, rij.y, rij.z, wij.x, wij.y, wij.z, wijsum.x, wijsum.y, wijsum.z);
				float3 M1, M2, M3;
				dd_Calculate_fijn_bpm_2(_1d_iL, Sc, fs, kdx, nij, rijm, m_E, b_r);
				...
				dd_Calculate_M123_bpm_2(R, W, Rij, kdx, idx, jdx, N, dt, nij, M1, M2, M3);
				...
				dd_Calculate_mijn_bpm_2(W, _1d_iL, Mijn, ms, kdx, idx, jdx, N, nij, M1, M2, M3, dt, m_G, b_r);
				
				//Mc = 0*__fsqrt_rn(Mc) * 2.0f * b_r;
				//Mijn[kdx].x = 0; Mijn[kdx].y = 0; Mijn[kdx].z = 0;
				//if(kdx == 35763)printf("In %u %u %u | %e %e %e\n", kdx, idx, jdx, Rij[kdx].x, Rij[kdx].y, Rij[kdx].z);
				dd_Calculate_mijt_bpm_2(W, _1d_iL, Mijt, ms, kdx, idx, jdx, N, nij, M1, M2, M3, dt, m_E, b_r);

				dd_Calculate_fijt_mijunsym_bpm_2(R, V, W, _1d_iL, Oijt, Mc, fs, ms, kdx, idx, jdx, N, nij, M1, M2, M3, rijm, dt, m_G, b_r);
				//Fijt[kdx].x = 0; Fijt[kdx].y = 0; Fijt[kdx].z = 0;
				//printf("In %u %u %u | %e %e %e | %e %e %e\n", kdx, idx, jdx, Fijt[kdx].x, Fijt[kdx].y, Fijt[kdx].z, Mijadd[kdx].x, Mijadd[kdx].y, Mijadd[kdx].z);
				//printf("In %u %u %u | %e %e\n", kdx, idx, jdx, Sc, Mc);
				dd_CheckBreakConditionDEM_2(Sc, Mc, Oijt, Mijn, Mijt, ILtype, kdx, m_Gcritn, m_Gcritt, b_r);
			}
		}
		F[idx] += fs.x;
		F[idx + N] += fs.y;
