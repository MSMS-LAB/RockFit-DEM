...
__device__ void dd_FrictionForce2(const float* __restrict__ V, float* __restrict__ F, const uint_fast32_t &N, const uint_fast32_t& idx, const float &fz, const float& mu)
{
	float vv = V[idx] * V[idx] + V[idx + N] * V[idx + N];
	if (vv > 1e-10)
	{
		float _1d_v = __frsqrt_rn(vv);
		F[idx] -= fabsf(fz) * mu * V[idx] * _1d_v;
		F[idx + N] -= fabsf(fz) * mu * V[idx + N] * _1d_v;
	}
}
...
__global__ void d_UniaxialCompression2_simple(float* __restrict__ R, const float* __restrict__ V, float* __restrict__ F, float* __restrict__ FL, const uint_fast32_t N, const float3 c, const float RR, const float Hd2, const float C, const float mu, const float Zcut)
{
	...
	while (idx < N)
	{
		...
		if (r.x * r.x + r.y * r.y < RR && R[idx + 2 * N] < Zcut)
		{
			if (r.z - Hd2 > 0)
			{				
				...
				if(fz > 0)
				{					
					//dd_FrictionForce2(V, F, N, idx, fz, mu);
					sft += fz;
					R[idx + 2 * N] = c.z + Hd2;
				}
