...
__device__ void dd_CheckBreakConditionDEM_5(float& Sc, float& Mc, float3* __restrict__ Oijt,	float3* __restrict__ Mijn, float3* __restrict__ Mijt,
	uint_fast8_t* __restrict__ ILtype, const uint_fast32_t& kdx, const float m_Gcritn, const float m_Gcritt, const float b_r)
{
	Mc *= norm3df(Oijt[kdx].x, Oijt[kdx].y, Oijt[kdx].z);
	Sc += norm3df(Mijt[kdx].x, Mijt[kdx].y, Mijt[kdx].z) * 4.0f * b_r;
	Mc += norm3df(Mijn[kdx].x, Mijn[kdx].y, Mijn[kdx].z) * 2.0f * b_r;
	if ((Sc > m_Gcritn) || (Mc > m_Gcritt))
	{
		ILtype[kdx] = 0;
...
__device__ void dd_Calculate_rijm_nij_5(const float* __restrict__ R, const uint_fast32_t& idx, const uint_fast32_t& jdx, const uint_fast32_t& N, float3& nij, float& rijm)
{
	float3 rij;
	float _1d_rm;
	rij.x = R[jdx] - R[idx];
...
__device__ void dd_Calculate_M123_bpm_5(const float* __restrict__ R, const float* __restrict__ W, float3* __restrict__ Rij, const uint_fast32_t& kdx, const uint_fast32_t& idx, const uint_fast32_t& jdx, const uint_fast32_t& N, const float dt, const float3& nij, float3& M1, float3& M2, float3& M3)
{
...
__device__ void dd_Calculate_fijn_bpm_5(const float* __restrict__ _1d_iL, float& Sc, float3& Fijn, const uint_fast32_t& kdx, const float3& nij, const float& rijm, const float& m_E, const float& b_r)
{
	// calculate the force
	...
	float cef =  m_E * (rijm - __frcp_rn(_1d_iL[kdx])) * _1d_iL[kdx];
	Sc = cef;
	cef *= b_r * b_r * MCf_pi;
	Fijn.x += nij.x * cef;
	Fijn.y += nij.y * cef;
...
__device__ void dd_Calculate_mijn_bpm_5(const float* __restrict__ W, const float* __restrict__ _1d_iL, float3* __restrict__ Mijn, float3& Mijnn, const uint_fast32_t& kdx, const uint_fast32_t& idx, const uint_fast32_t& jdx, const uint_fast32_t& N,
	const float3& nij, const float3& M1, const float3& M2, const float3& M3, const float dt, const float& m_G, const float& b_r)
{
	...
	cef1 *= dt * 0.5f * m_G * _1d_iL[kdx];
	float3 mijn;
	mijn.x = M1.x * Mijn[kdx].x + M1.y * Mijn[kdx].y + M1.z * Mijn[kdx].z + nij.x * cef1;
	mijn.y = M2.x * Mijn[kdx].x + M2.y * Mijn[kdx].y + M2.z * Mijn[kdx].z + nij.y * cef1;
	mijn.z = M3.x * Mijn[kdx].x + M3.y * Mijn[kdx].y + M3.z * Mijn[kdx].z + nij.z * cef1;
	//Mc = (mijn.x * mijn.x + mijn.y * mijn.y + mijn.z * mijn.z);
	Mijn[kdx].x = mijn.x;
	Mijn[kdx].y = mijn.y;
	Mijn[kdx].z = mijn.z;
	//Mc += (nij.x * mijn.x + nij.y * mijn.y + nij.z * mijn.z) * 2.0f * b_r;
	
	cef1 = MCf_pi * b_r * b_r * b_r * b_r;
	Mijnn.x += mijn.x * cef1;
	Mijnn.y += mijn.y * cef1;
	Mijnn.z += mijn.z * cef1;
...
__device__ void dd_Calculate_mijt_bpm_5(const float* __restrict__ W, const float* __restrict__ _1d_iL, float3* __restrict__ Mijt, float3& Mijtt, const uint_fast32_t& kdx, const uint_fast32_t& idx, const uint_fast32_t& jdx, const uint_fast32_t& N,
	const float3& nij, const float3& M1, const float3& M2, const float3& M3, const float dt, const float& m_E, const float& b_r)
{
	...
	float cef1 = nij.x * (W[jdx] - W[idx]) + nij.y * (W[jdx + N] - W[idx + N]) + nij.z * (W[jdx + 2 * N] - W[idx + 2 * N]);
	float cef2 = dt * 0.25f * m_E * _1d_iL[kdx];
	float3 mijt;
	...
	//S += norm3df(mijt.x, mijt.y, mijt.z) * b_r;	
	Mijt[kdx].x = mijt.x;
	Mijt[kdx].y = mijt.y;
	Mijt[kdx].z = mijt.z;
	cef2 *= MCf_pi * b_r * b_r * b_r * b_r;
	Mijtt.x += mijt.x * cef2;
	Mijtt.y += mijt.y * cef2;
	Mijtt.z += mijt.z * cef2;	
}

__device__ void dd_Calculate_fijt_mijunsym_bpm_5(const float* __restrict__ R, const float* __restrict__ V, const float* __restrict__ W, 
	const float* __restrict__ _1d_iL, float3* __restrict__ Oijt, float& Mc, float3& Fijt, float3& Mijadd, const uint_fast32_t& kdx, const uint_fast32_t& idx, const uint_fast32_t& jdx, const uint_fast32_t& N,
	const float3& nij, const float3& M1, const float3& M2, const float3& M3, const float& rijm, const float dt, const float& m_G, const float& b_r)
{
	...
	Mijadd.x += 0.5f * (rij.y * oijt.z - rij.z * oijt.y) * cef;
	Mijadd.y += 0.5f * (rij.z * oijt.x - rij.x * oijt.z) * cef;
	Mijadd.z += 0.5f * (rij.x * oijt.y - rij.y * oijt.x) * cef;
}

__device__ void dd_Calculate_fijn_ppHM_5(const float* __restrict__ V, const float* __restrict__ W, float3& Fijn,
	...
	const float3& nij, const float& rijm, float3& vijt, float& Kt, float& fijnm)
{
	...
	Fijn.x += nij.x * (fijnm + cef1);
	Fijn.y += nij.y * (fijnm + cef1);
	Fijn.z += nij.z * (fijnm + cef1);
...
__device__ void dd_Calculate_fijtmijt_ppHM_5(float3* __restrict__ Oijt, float3& Fijt, float3& Mijt,
	...
	const float3& vijt, const float& Kt, const float& fijnm)
{
	...
	Fijt.y += fijt.y;
	Fijt.z += fijt.z;
	Mijt.x += (nij.y * fijt.z - nij.z * fijt.y) * rP;
	Mijt.y += (nij.z * fijt.x - nij.x * fijt.z) * rP;
	Mijt.z += (nij.x * fijt.y - nij.y * fijt.x) * rP;
}

__device__ void dd_Calculate_mijroll_ppHM_5(const float* __restrict__ W, float3& Mijadd, const uint_fast32_t& kdx, const uint_fast32_t& idx, const uint_fast32_t& N, const float& m_muroll, const float& fijnm)
{
	float3 wi;
	wi.x = W[idx];
	wi.y = W[idx + N];
...
__global__ void d_CalculateForcesDEM_51(const float* __restrict__ R, const float* __restrict__ V,
	...
	while (idx < N)
	{
		...
		for (kdx = idx * IonP; kdx < (idx + 1) * IonP; ++kdx)
		{
			...
			if (jdx != UINT_FAST32_MAX && ILtype[kdx] == 0)
			{
				float3 nij;
				float rijm;
				dd_Calculate_rijm_nij_5(R, idx, jdx, N, nij, rijm);
				if (rijm < 2.0f * rP)
				{
					...
					float Kt, fijnm;
					//CHECK Overlap!
					dd_Calculate_fijn_ppHM_5(V, W, fs, kdx, idx, jdx, N, m_E, m_G, p_A, mP, rP, nij, rijm, vijt, Kt, fijnm);
					//printf("pp %e %e %e %e %e\n", 1.0f*mP, 1.0f*rP);
					...
					//Fijn[kdx].y = 0;
					//Fijn[kdx].z = 0;
					dd_Calculate_fijtmijt_ppHM_5(Oijt, fs, ms, kdx, nij, m_mu, p_A, mP, rP, dt, vijt, Kt, fijnm);
					//printf("pp %e %e %e |\n", Kt);
					//Fijt[kdx].x = 0;
					//Fijt[kdx].y = 0;
					//Fijt[kdx].z = 0;
					...
					//Mijt[kdx].y = 0;
					//Mijt[kdx].z = 0;
					dd_Calculate_mijroll_ppHM_5(W, ms, kdx, idx, N, m_muroll, fijnm);
					//Mijadd[kdx].x = 0;
					//Mijadd[kdx].y = 0;
					//Mijadd[kdx].z = 0;	
				}
...
__global__ void d_CalculateForcesDEM_52(const float* __restrict__ R, const float* __restrict__ V,
	const float* __restrict__ W, const	uint_fast32_t* __restrict__ IL, uint_fast8_t* __restrict__ ILtype,
	...
	const	uint_fast32_t N, const uint_fast32_t IonP, const float dt, const float m_E, const float m_G, const float b_r, const float m_Gcritn, const float m_Gcritt)
{
	...
	{		
		...
		{
			jdx = IL[kdx];			
			if (jdx != UINT_FAST32_MAX && ILtype[kdx] == 1)
			{
				//Sc = 0; Mc = 0;
				float3 nij;
				float rijm;
				...
				//printf("In %u %u %u | %e %e %e | %e %e %e | %e %e %e\n", kdx, idx, jdx, rij.x, rij.y, rij.z, wij.x, wij.y, wij.z, wijsum.x, wijsum.y, wijsum.z);
				float3 M1, M2, M3;
				dd_Calculate_fijn_bpm_5(_1d_iL, Sc, fs, kdx, nij, rijm, m_E, b_r);
				...
				dd_Calculate_M123_bpm_5(R, W, Rij, kdx, idx, jdx, N, dt, nij, M1, M2, M3);
				...
				dd_Calculate_mijn_bpm_5(W, _1d_iL, Mijn, ms, kdx, idx, jdx, N, nij, M1, M2, M3, dt, m_G, b_r);
				
				//Mc = 0*__fsqrt_rn(Mc) * 2.0f * b_r;
				//Mijn[kdx].x = 0; Mijn[kdx].y = 0; Mijn[kdx].z = 0;
				//if(kdx == 35763)printf("In %u %u %u | %e %e %e\n", kdx, idx, jdx, Rij[kdx].x, Rij[kdx].y, Rij[kdx].z);
				dd_Calculate_mijt_bpm_5(W, _1d_iL, Mijt, ms, kdx, idx, jdx, N, nij, M1, M2, M3, dt, m_E, b_r);

				dd_Calculate_fijt_mijunsym_bpm_5(R, V, W, _1d_iL, Oijt, Mc, fs, ms, kdx, idx, jdx, N, nij, M1, M2, M3, rijm, dt, m_G, b_r);
				//Fijt[kdx].x = 0; Fijt[kdx].y = 0; Fijt[kdx].z = 0;
				//printf("In %u %u %u | %e %e %e | %e %e %e\n", kdx, idx, jdx, Fijt[kdx].x, Fijt[kdx].y, Fijt[kdx].z, Mijadd[kdx].x, Mijadd[kdx].y, Mijadd[kdx].z);
				//printf("In %u %u %u | %e %e\n", kdx, idx, jdx, Sc, Mc);
				dd_CheckBreakConditionDEM_5(Sc, Mc, Oijt, Mijn, Mijt, ILtype, kdx, m_Gcritn, m_Gcritt, b_r);
			}
		}
		F[idx] += fs.x;
		F[idx + N] += fs.y;
